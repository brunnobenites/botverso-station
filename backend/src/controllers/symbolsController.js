...
async function updateSymbol(req, res, next) {
  ...
  if (origin === process.env.BEHOLDER_URL) {
    ...
    else await favoriteSymbolsRepository.deleteFavorite(symbol, userId);
...
async function getSymbols(req, res, next) {
  ...
  let symbols = {};
  if (search || page)
    symbols = await symbolsRepository.searchSymbols(
      search,
      page,
  ...
  if (origin === process.env.BEHOLDER_URL) {
    const favoriteObjects = await favoriteSymbolsRepository.getFavorites(
      userId
    );
...
async function getSymbol(req, res, next) {
  ...
  if (symbol.startsWith("*"))
    return res.json({ symbol, base: "*", quote: symbol.replace("*", "") });
...
async function syncSymbols(req, res, next) {
  ...
  const ignoredCoins = process.env.IGNORED_COINS
    ? process.env.IGNORED_COINS.split(",")
    : [];
  ...
  (await exchange.futuresExchangeInfo()).symbols.map((item) => {
    for (let i = 0; i < symbols.length; i++) {
      if (symbols[i].symbol === item.symbol) {
        ...
        symbols[i].fMinNotional = minNotionalFilter
          ? minNotionalFilter.notional
          : "1";
        symbols[i].fMinLotSize = minLotSizeFilter
          ? minLotSizeFilter.minQty
          : "1";
        symbols[i].fStepSize = minLotSizeFilter
          ? minLotSizeFilter.stepSize
          : "1";

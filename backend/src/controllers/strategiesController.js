...
async function getStrategies(req, res, next) {
  ...
  let result;
  if (symbol)
    result = await strategiesRepository.getStrategiesBySymbol(
      userId,
      symbol,
      page
    );
  else result = await strategiesRepository.getStrategies(userId, page);
  res.json(result);
}
...
async function getSharedStrategies(req, res, next) {
  ...
  const result = await strategiesRepository.getSharedStrategies(
    userId,
    user.email,
    includePublic,
    symbol,
...
async function startStrategyComponents(strategy) {
  ...
  if (!monitor.isSystemMon) monitorsController.startMonitorExecution(monitor);

  const buyAutomation = await automationsRepository.getAutomation(
    strategy.buyAutomationId
  );
  automationsController.startAutomationExecution(buyAutomation);

  if (strategy.buyAutomationId !== strategy.sellAutomationId) {
    const sellAutomation = await automationsRepository.getAutomation(
      strategy.sellAutomationId
    );
...
async function stopStrategyComponents(strategy) {
  const monitor = await monitorsRepository.getMonitor(strategy.monitorId);
  if (!monitor.isSystemMon) monitorsController.stopMonitorExecution(monitor);
...
async function insertStrategy(req, res, next) {
  ...
  if (newStrategy.isActive) newStrategy.startedAt = new Date();
  else newStrategy.startedAt = null;
...
async function updateStrategy(req, res, next) {
  ...
  const updatedStrategy = await strategiesRepository.updateStrategy(
    id,
    newStrategy
  );

  if (currentStrategy.isActive) await stopStrategyComponents(currentStrategy);

  if (updatedStrategy.isActive) await startStrategyComponents(updatedStrategy);
...
async function deleteStrategy(req, res, next) {
  ...
  if (currentStrategy.isActive) stopStrategyComponents(currentStrategy);
...
async function copyMonitor(monitor, userId, transaction) {
  ...
  if (["CANDLES", "TICKER"].includes(monitor.type)) {
    ...
    monitorCopy = await monitorsRepository.insertMonitor(
      monitorCopy,
      transaction
    );
...
async function copyAutomation(automation, userId, transaction) {
  ...
  const promises = automation.actions.map(async (action) => {
    if (action.orderTemplateId) {
      ...
      delete wt.id;
      wt.userId = userId;
      wt = await withdrawTemplatesRepository.insertWithdrawTemplate(
        wt,
        transaction
      );
  ...
  const isGrid =
    automationCopy.actions[0].type === actionsRepository.actionTypes.GRID;
  let levels, quantity;
  if (isGrid) {
    levels = automation.grids.length + 1;
    quantity = automation.grids[0].orderTemplate.quantity;
  }
  automation = await automationsController.saveAutomation(
    automationCopy,
    levels,
    quantity,
    transaction
...
async function copyStrategy(req, res, next) {
  ...
  if (strategy.sharedWith !== "everyone" && !sharedWith.includes(user.email))
    return res.sendStatus(403);
  if (user.automations.length >= user.limit.maxAutomations - 1)
    return res
      .status(409)
      .json(`You have reached the max automations in your plan.`);
  if (user.monitors.length >= user.limit.maxMonitors)
    return res
      .status(409)
      .json(`You have reached the max monitors in your plan.`);
  ...
  try {
    const monitor = await monitorsRepository.getMonitor(strategy.monitorId);
    newStrategy.monitorId = await copyMonitor(
      monitor.get({ plain: true }),
      userId,
      transaction
    );
    ...
    const buyAutomationId = await copyAutomation(
      buyAutomation,
      userId,
      transaction
    );
    ...
    if (strategy.buyAutomationId !== strategy.sellAutomationId) {
      ...
      const sellAutomationId = await copyAutomation(
        sellAutomation,
        userId,
        transaction
      );

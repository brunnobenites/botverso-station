...
async function startUserMonitors(user) {
  ...
  const userDataMonitor = systemMonitors.find(
    (m) => m.type === monitorTypes.USER_DATA
  );
  const futuresMonitor = systemMonitors.find(
    (m) => m.type === monitorTypes.FUTURES_DATA
  );

  if (!user.monitors) user.monitors = [];
  if (userDataMonitor && user.accessKey && user.secretKey)
    user.monitors.push(userDataMonitor);
  if (futuresMonitor && user.futuresKey && user.futuresSecret)
    user.monitors.push(futuresMonitor);
      ...
      setTimeout(() => {
        switch (m.type) {
          case monitorTypes.USER_DATA: {
            const decryptedUser = {
              ...user.get({ plain: true }),
              secretKey: crypto.decrypt(user.secretKey),
            };
            return appEm.startUserDataMonitor(
              decryptedUser,
              m.id,
              m.broadcastLabel,
              m.logs
          ...
          case monitorTypes.FUTURES_DATA: {
            const decryptedUser = {
              ...user.get({ plain: true }),
              futuresSecret: crypto.decrypt(user.futuresSecret),
            };
            return appEm.startFuturesMonitor(
              decryptedUser,
              m.id,
              m.broadcastLabel,
              m.logs
          ...
          case monitorTypes.CANDLES:
            return appEm.startChartMonitor(
              user.id,
              m.id,
              m.symbol,
              m.interval,
...
async function stopUserMonitors(user) {
  ...
  if (!user.monitors) user.monitors = [];
  if (user.accessKey && user.secretKey && userDataMonitor)
    user.monitors.push(userDataMonitor);
  if (user.futuresKey && user.futuresSecret && futuresMonitor)
    user.monitors.push(futuresMonitor);
  ...
  await Promise.all(
    user.monitors.map(async (monitor) => {
      ...
      if (
        ![monitorTypes.USER_DATA, monitorTypes.FUTURES_DATA].includes(
          monitor.type
        )
      )
...
async function sendResetAlerts(user, newPassword) {
  ...
  if (settings.sendGridKey)
    promises.push(
      email(
        ...
        Your password at Botverso Station was resetted.
        ...
        `,
        user.email,
        "Botverso Station password resetted!"
      )
    );
  ...
  if (user.phone && settings.twilioSid)
    promises.push(
      sms(
        settings,
        `Your password at Botverso Station was resetted. Look your email!`,
        user.phone
      )
    );
...
async function sendWelcomeAlerts(user, newPassword) {
  ...
  if (settings.sendGridKey)
    promises.push(
      email(
        ...
        Your account at Botverso Station was created.
        ...
        `,
        user.email,
        "Botverso Station account created!"
      )
    );
  ...
  if (user.phone && settings.twilioSid)
    promises.push(
      sms(
        settings,
        `Your account at Botverso Station was created. Look your email!`,
        user.phone
      )
    );
...
async function sendStopAlerts(user) {
  ...
  const fullText = `Hello ${user.name.split(" ")[0]},
    ...
    Your account at Botverso Station was stopped.

    Sorry!`;

  if (settings.sendGridKey)
    promises.push(
      email(settings, fullText, user.email, "Botverso Station account stopped!")
    );

  const shortText = `Your account at Botverso Station was stopped. Sorry!`;
  ...
  if (user.pushToken)
    promises.push(
      push.send(user, shortText, "Botverso Station Notification", {
        text: shortText,
        type: "error",
      })
    );
...
async function sendStartAlerts(user) {
  ...
  const fullText = `Hello ${user.name.split(" ")[0]},
    ...
    Your account at Botverso Station was (re)started.

    Enjoy!`;

  if (settings.sendGridKey)
    promises.push(
      email(settings, fullText, user.email, "Botverso Station account started!")
    );

  const shortText = `Your account at Botverso Station was (re)started. Enjoy!`;
...
async function stopUserAutomations(user) {
  ...
  await Promise.all(
    user.automations.map(async (automation) => {
      if (!automation.isActive) return;

      if (automation.schedule) agenda.cancelSchedule(automation.id);
      else hydra.deleteBrain(automation.get({ plain: true }));

      await automationsRepository.updateAutomation(automation.id, {
        isActive: false,
      });
      ...
      if (automation.logs)
        logger(
          `A:${automation.id}`,
          `Automation ${automation.name} has stopped!`
        );
    })
  );
}
...
async function deleteUser(req, res, next) {
  ...
  try {
    ...
    return res
      .status(500)
      .json(`There was an error to delete the user. Check the system logs.`);
  }
}

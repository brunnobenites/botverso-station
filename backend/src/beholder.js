...
module.exports = class Beholder {
  ...
  constructor(automations, testCache = false) {
    ...
    setTimeout(() => {
      automations
        .filter((auto) => auto.isActive && !auto.schedule)
        .map((auto) => this.updateBrain(auto));
      logger(
        "beholder",
        `Beholder Brain has started for user ${automations[0].userId}!`
      );
    }, 1000);
  }
  ...
  isLocked(automationId) {
    if (Array.isArray(automationId))
      return Object.keys(this.LOCK_BRAIN).some(
        (k) => this.LOCK_BRAIN[k] === true
      );
  ...
  updateBrainIndex(index, automation) {
    ...
    this.listener.subscribe(index, async (message) => {
      ...
      if (results && results.length) {
        results.map((r) =>
          appEm.sendMessage(automation.userId, { notification: r })
        );
      }
    });
  }
  ...
  updateBrain(automation) {
    ...
    automation.indexes
      .split(",")
      .map((ix) => this.updateBrainIndex(ix, automation));
  }
  ...
  async sendSms(settings, user, automation) {
    ...
    await require("./utils/sms")(
      settings,
      automation.name + " has fired!",
      user.phone
    );
  ...
  async sendEmail(settings, user, automation, subject) {
    ...
    return {
      text: `E-mail sent from automation '${automation.name}'`,
      type: "success",
    };
  }
  ...
  async calcPrice(orderTemplate, symbol, isStopPrice, position = null) {
    ...
    const tickSize = isFuture
      ? parseFloat(symbol.fTickSize)
      : parseFloat(symbol.tickSize);
    let newPrice, factor;
  ...
  async calcQty(orderTemplate, price, symbol, position = null) {
    ...
    if (orderTemplate.side === "BUY" || isFuture) {
      asset = parseFloat(
        await this.cache.get(
          `${symbol.quote}:${walletIndex}_${orderTemplate.userId}`
        )
      );
      if (!asset)
        throw new Error(
          `There is no ${symbol.quote} in your wallet to place the order.`
        );
    } else {
      asset = parseFloat(
        await this.cache.get(
          `${symbol.base}:${walletIndex}_${orderTemplate.userId}`
        )
      );
      if (!asset)
        throw new Error(
    ...
    const stepSize = isFuture
      ? parseFloat(symbol.fStepSize)
      : parseFloat(symbol.stepSize);
    ...
    if (/^(MAX_F?WALLET)$/.test(orderTemplate.quantity)) {
      ...
      else newQty = parseFloat(asset) * (multiplier > 1 ? 1 : multiplier);
    } else if (orderTemplate.quantity === "MIN_NOTIONAL") {
      const minNotional = isFuture
        ? parseFloat(symbol.fMinNotional)
        : parseFloat(symbol.minNotional);
      newQty =
        (minNotional / parseFloat(price)) * (multiplier < 1 ? 1 : multiplier);
    } else if (orderTemplate.quantity === "POSITION_AMT") {
      if (!position)
        throw new Error(
          `There is no position to use as qty reference for ${orderTemplate.symbol}.`
        );
      ...
      const lastOrder = await this.cache.get(
        `${orderTemplate.symbol}:${lastOrderIndex}_${orderTemplate.userId}`
      );
      if (!lastOrder)
        throw new Error(
      ...
      if (!isFuture && orderTemplate.side === "SELL" && newQty > asset)
        newQty = asset;
    }
  ...
  async hasEnoughAssets(userId, symbol, order, price) {
    ...
    if (order.side === "BUY")
      return (
        parseFloat(await this.cache.get(`${symbol.quote}:WALLET_${userId}`)) >=
        price * qty
      );
    else
      return (
        parseFloat(await this.cache.get(`${symbol.base}:WALLET_${userId}`)) >=
        qty
      );
  }
  ...
  async execTest(automation, order, symbol) {
    ...
    order.quoteOrderQuantity = order.options.quoteOrderQty
      ? parseFloat(order.options.quoteOrderQty)
      : 0;

    const book = await this.cache.get(`${automation.symbol}:BOOK`);
    const currentPrice =
      order.side === "BUY" ? book.current.bestAsk : book.current.bestBid;
    const orderValue = order.quoteOrderQuantity
      ? order.quoteOrderQuantity
      : order.quantity * currentPrice;
    const commission = orderValue * 0.001; //ou 0.00075 com BNB

    const quoteAsset = await this.cache.get(
      `${symbol.quote}:WALLET_${automation.userId}`
    );
    const baseAsset = await this.cache.get(
      `${symbol.base}:WALLET_${automation.userId}`
    );
    const baseQuantity = order.quantity
      ? order.quantity
      : order.quoteOrderQuantity / currentPrice;

    if (order.side === "BUY") {
      await this.cache.set(
        `${symbol.quote}:WALLET_${automation.userId}`,
        quoteAsset - orderValue - commission
      );
      await this.cache.set(
        `${symbol.base}:WALLET_${automation.userId}`,
        baseAsset + baseQuantity
      );
    } else {
      await this.cache.set(
        `${symbol.quote}:WALLET_${automation.userId}`,
  ...
  async calcQuoteQty(orderTemplate, symbol) {
    if (orderTemplate.type !== "MARKET" || parseFloat(orderTemplate.quantity))
      throw new Error(`Only MARKET orders can calc quote qty.`);
    ...
    if (orderTemplate.quantity === "MAX_WALLET") {
      if (orderTemplate.side !== "BUY")
        throw new Error(
          `Only MARKET BUY orders can calc quote qty with MAX_WALLET.`
        );

      const asset = parseFloat(
        await this.cache.get(`${symbol.quote}:WALLET_${orderTemplate.userId}`)
      );
      if (!asset)
        throw new Error(
      ...
      return (
        parseFloat(symbol.minNotional) * (multiplier < 1 ? 1 : multiplier)
      ).toFixed(symbol.quotePrecision);
  ...
  async calcStopPrice(orderTemplate, symbol, position = null) {
    const stopPrice = await this.calcPrice(
      orderTemplate,
      symbol,
      true,
      position
  ...
  async adjustMarginType(orderTemplate, exchange, position) {
    ...
    if (!position)
      return exchange.futuresMargin(
        orderTemplate.symbol,
        orderTemplate.marginType
      );

    const currentMargin =
      position.marginType === "cross" ? "CROSSED" : "ISOLATED";
  ...
  async adjustLeverage(orderTemplate, exchange, position) {
    ...
    if (!position)
      return exchange.futuresLeverage(
        orderTemplate.symbol,
        orderTemplate.leverage
      );
  ...
  async placeOrder(user, automation, action, symbolObject = false) {
    ...
    const orderTemplate = action.orderTemplate
      ...
      : (
          await orderTemplatesRepository.getOrderTemplate(
            user.id,
    ...
    const symbol = symbolObject
      ? symbolObject
      : await getSymbol(orderTemplate.symbol);
    ...
    const isFuture =
      orderTemplate.leverage !== undefined && orderTemplate.leverage !== null;

    let position = null;
    if (isFuture)
      position = await this.cache.get(
        `${automation.symbol}:POSITION_${automation.userId}`
      );

    const dynamicPrices = ["MIN_NOTIONAL", "MAX_WALLET", "QUOTE_QTY"];
    const isDynamicBuy =
      order.side === "BUY" && dynamicPrices.includes(orderTemplate.quantity);
    if (
      !isFuture &&
      ...
      if (!isFinite(price) || !price)
        throw new Error(
          `Error in calcPrice function, params: OTID ${
            orderTemplate.id
          }, $: ${price}, stop: false, position: ${JSON.stringify(position)}`
        );

      if (LIMIT_TYPES.includes(order.options.type)) order.limitPrice = price;

      const quantity = await this.calcQty(
        orderTemplate,
        price,
        symbol,
        position
      ...
      if (!isFinite(quantity) || !quantity)
        throw new Error(
          `Error in calcQty function, params: OTID ${
            orderTemplate.id
          }, $: ${price}, qty: ${quantity}, position: ${JSON.stringify(
            position
          )}`
        );
      ...
      if (STOP_TYPES.includes(order.options.type))
        order.options.stopPrice = await this.calcStopPrice(
          orderTemplate,
          symbol,
          position
        );
      ...
      if (!isFuture) {
        const hasEnough = await this.hasEnoughAssets(
          user.id,
          symbol,
          order,
          price
        ...
        if (!hasEnough)
          throw new Error(
            `You wanna ${order.side} ${order.quantity} ${order.symbol} but you haven't enough assets.`
          );
      } else {
        if (order.options.type === "TRAILING_STOP_MARKET") {
          order.options.activationPrice = await this.calcPrice(
            orderTemplate,
            symbol,
            false,
            position
        ...
        if (
          orderTemplate.reduceOnly !== null &&
          orderTemplate.reduceOnly !== undefined
        )
    ...
    try {
      if (isFuture) {
        if (orderTemplate.leverage > 0) {
          const leverageResult = await this.adjustLeverage(
            orderTemplate,
            exchange,
            position
          );
          if (leverageResult.code !== undefined && leverageResult.code < 0)
            throw new Error(leverageResult.msg);
        }

        if (orderTemplate.marginType) {
          const marginResult = await this.adjustMarginType(
            orderTemplate,
            exchange,
            position
          );
          if (marginResult.code !== undefined && marginResult.code < 0)
            throw new Error(marginResult.msg);
        }
      }

      if (order.side === "BUY") {
        if (isFuture)
          result = await exchange.futuresBuy(
            order.symbol,
            order.quantity,
            order.limitPrice,
            order.options
        ...
        if (isFuture)
          result = await exchange.futuresSell(
            order.symbol,
            order.quantity,
            order.limitPrice,
            order.options
    ...
    return {
      type: "success",
      text: `Order ${savedOrder.side} ${savedOrder.symbol} ${savedOrder.status}`,
    };
  }
  ...
  async gridEval(settings, user, automation) {
    ...
    if (LOGS)
      logger(
        "A:" + automation.id,
        `Beholder is in the GRID zone at ${automation.name}`
      );
    ...
    for (let i = 0; i < automation.grids.length; i++) {
      ...
      if (automation.logs)
        logger(
          "A:" + automation.id,
          `Beholder evaluated a condition at ${automation.name} => ${grid.conditions}`
        );
      ...
      const book = await this.cache.get(`${automation.symbol}:BOOK`);
      if (!book)
        return { type: "error", text: `No book info for ${automation.symbol}` };
      ...
      if (automation.logs && user.telegramChat)
        await require("./utils/telegram")(
          settings,
          result.text,
          user.telegramChat
        );
      ...
      try {
        ...
        return {
          type: "error",
          text: `Beholder can't generate grids for ${automation.name}. ERR: ${err.message}`,
        };
      }
  ...
  async generateGrids(automation, levels, quantity, transaction) {
    ...
    const orderTemplates =
      await orderTemplatesRepository.getOrderTemplatesByGridName(
        automation.userId,
        automation.name
      );
    ...
    const book = await this.cache.get(`${automation.symbol}:BOOK`);
    if (!book)
      throw new Error(`There is no book info for ${automation.symbol}`);
    ...
    for (let i = 1; i <= levels; i++) {
      ...
      if (targetPrice < currentPrice) {
        ...
        //se está acima da cotação, vende
  ...
  async withdrawCrypto(user, automation, action) {
    ...
    if (!action.withdrawTemplateId)
      throw new Error(
        `There is no withdraw template for '${automation.name}', action #${action.id}`
      );

    const withdrawTemplate =
      await withdrawTemplatesRepository.getWithdrawTemplate(
        action.withdrawTemplateId
      );
    ...
    if (!amount) {
      if (withdrawTemplate.amount === "MAX_WALLET") {
        const available = await this.cache.get(
          `${withdrawTemplate.coin}:WALLET_${user.id}`
        );
        if (!available) throw new Error(`No available funds for this coin.`);

        amount =
          available *
          (withdrawTemplate.amountMultiplier > 1
            ? 1
            : withdrawTemplate.amountMultiplier);
      } else if (withdrawTemplate.amount === "LAST_ORDER_QTY") {
        const keys = this.searchMemory(
          new RegExp(
            `^((${withdrawTemplate.coin}.+|.+${withdrawTemplate.coin}):LAST_ORDER_${user.id})$`
          )
        );
        if (!keys || !keys.length)
          throw new Error(`No last order for this coin.`);

        amount =
          keys[keys.length - 1].value.quantity *
          withdrawTemplate.amountMultiplier;
      }
    }
    ...
    try {
      const result = await exchange.withdraw(
        withdrawTemplate.coin,
        amount,
        withdrawTemplate.address,
        withdrawTemplate.network,
        withdrawTemplate.addressTag
      );

      if (automation.logs)
        logger("A:" + automation.id, `WITHDRAW`, withdrawTemplate);

      return {
        type: "success",
        text: `Withdraw #${result.id} realized successfully for ${withdrawTemplate.coin}`,
      };
    } catch (err) {
      throw new Error(
  ...
  async sendTelegram(settings, user, automation) {
    if (!user.telegramChat)
      throw new Error(`This user doesn't have Telegram Chat ID.`);
    ...
    return {
      text: `Telegram sent from automation '${automation.name}'`,
      type: "success",
    };
  }
  ...
  async trailingEval(user, automation, action) {
    ...
    if (!activactionPrice) {
      activactionPrice = await this.parsePrice(
        automation.symbol,
        action.orderTemplate.limitPrice,
        automation.userId
      );
    ...
    const isPriceActivated = isBuy
      ? currentPrice <= activactionPrice
      : currentPrice >= activactionPrice;
    ...
    if (LOGS)
      logger(
        "A:" + automation.id,
        `Beholder is in the Trailing zone at ${automation.name}`
      );

    const isStopActivated = isBuy
    ...
    if (isStopActivated) {
      if (automation.logs || LOGS)
        logger(
          "A:" + automation.id,
          `Stop price activated at ${automation.name}`
        );
    ...
    const newStopPrice = isBuy
      ? currentPrice *
        (1 + parseFloat(action.orderTemplate.stopPriceMultiplier) / 100)
      : currentPrice *
        (1 - parseFloat(action.orderTemplate.stopPriceMultiplier) / 100);

    if (
      !stopPrice ||
      (isBuy && newStopPrice < stopPrice) ||
      (!isBuy && newStopPrice > stopPrice)
    ) {
      if (LOGS)
        logger(
          "A:" + automation.id,
          `Stop price changed to ${newStopPrice} at ${automation.name}`
        );

      action.orderTemplate.stopPrice = newStopPrice;
      await orderTemplatesRepository.updateOrderTemplate(
        user.id,
        action.orderTemplate.id,
    ...
    try {
      switch (action.type) {
        case actionTypes.ALERT_EMAIL:
          return this.sendEmail(settings, user, automation);
        case actionTypes.ALERT_SMS:
          return this.sendSms(settings, user, automation);
        case actionTypes.ALERT_TELEGRAM:
          return this.sendTelegram(settings, user, automation);
        case actionTypes.ORDER:
          return this.placeOrder(user, automation, action);
        case actionTypes.WITHDRAW:
          return this.withdrawCrypto(user, automation, action);
        case actionTypes.GRID:
          return this.gridEval(settings, user, automation);
        case actionTypes.TRAILING:
          return this.trailingEval(user, automation, action);
      }
    } catch (err) {
      if (automation.logs) {
  ...
  async evalDecision(memoryKey, automation) {
    ...
    try {
      ...
      if (automation.logs && results.length)
        logger(
          "A:" + automation.id,
          `Results for automation ${automation.name} was ${JSON.stringify(
            results
          )} at ${new Date()}`
        );
    ...
    try {
      const promises = automations.map(async (automation) => {
        ...
        return this.evalDecision(
          realMemoryKey ? realMemoryKey : memoryKey,
          auto
        );
      });

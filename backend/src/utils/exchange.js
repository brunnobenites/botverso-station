...
function getApiUrl() {
  ...
  return getUrl(API_URL, "https://api.binance.com/api/");
}
...
function getFApiUrl() {
  ...
  return getUrl(FAPI_URL, "https://fapi.binance.com/fapi/");
}

function getStreamUrl() {
  const WS_URL = process.env.BINANCE_WS_URL;
  return getUrl(WS_URL, "wss://stream.binance.com:9443/ws/");
}

function getFStreamUrl() {
  const FWS_URL = process.env.BINANCE_FWS_URL;
  return getUrl(FWS_URL, "wss://fstream.binance.com/ws/");
}
...
module.exports = class Exchange {
  ...
  futuresPositions(symbol = "") {
    return this.binance.futuresPositionRisk(
      symbol ? { symbol, timestamp: Date.now() } : undefined
    );
  }
  ...
  withdraw(coin, amount, address, network, addressTag) {
    try {
      ...
      return this.privateCall(
        getSApiUrl() + "capital/withdraw/apply",
        data,
        "POST"
      );
    } catch (err) {
  ...
  async getCoins() {
    try {
      const coins = await this.privateCall(
        getSApiUrl() + "capital/config/getall",
        null,
        "GET"
      );
      return coins.map((c) => {
  ...
  async publicCall(apiUrl, data = {}, method = "GET") {
    if (!this.binance.APIKEY)
      throw new Error(`The APIKEY is required to connect on exchange!`);

    const axios = require("axios");
    const queryString = new URLSearchParams();
    Object.entries(data).map((prop) =>
      queryString.append(prop[0], `${prop[1]}`)
    );
  ...
  async privateCall(apiUrl, data = {}, method = "GET") {
    if (!this.binance.APIKEY || !this.binance.APISECRET)
      throw new Error(
        `The settings object is required to connect on exchange!`
      );
    ...
    const axios = require("axios");
    const queryString = new URLSearchParams();
    Object.entries({ ...data, timestamp, recvWindow }).map((prop) =>
      queryString.append(prop[0], `${prop[1]}`)
    );
  ...
  futuresUserDataStream(
    ...
    this.binance.websockets.userFutureData(
      ...
      (data) => {
        logger(
          "system",
          `futuresUserDataStream:subscribeEvent: ${JSON.stringify(data)}`
        );

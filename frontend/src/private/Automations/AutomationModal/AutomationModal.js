...
import {
  getIndexes,
  getDefaultIndexes,
  getFuturesLastOrderIndexes,
  getFuturesLiquidationIndexes,
  getLastOrderIndexes,
} from "../../../services/BeholderService";
  ...
  useEffect(() => {
    ...
    if (automationSymbol.startsWith("*"))
      ...
      getSymbol(automationSymbol)
        .then((symbolObj) => {
          if (symbolObj) setSymbol(symbolObj);
          else setIndexes(getDefaultIndexes(automationSymbol));
        })
        .catch((err) => {
          console.error(err.response ? err.response.data : err.message);
          setError(err.response ? err.response.data : err.message);
  ...
  useEffect(() => {
    ...
    function getWalletIndexes(indexes, symbol) {
      ...
      const quoteWallet = indexes.find(
        (ix) =>
          ix.variable.startsWith("WALLET_") &&
          symbol.quote === ix.symbol.replace("*", "")
      );
      if (quoteWallet) walletIndexes.push(quoteWallet);

      if (localStorage.getItem("hasFutures") === "true") {
        const baseFWallet = indexes.find(
          (ix) =>
            ix.variable.startsWith("FWALLET_") && symbol.base === ix.symbol
        );
        if (baseFWallet) walletIndexes.push(baseFWallet);

        const quoteFWallet = indexes.find(
          (ix) =>
            ix.variable.startsWith("FWALLET_") &&
            symbol.quote === ix.symbol.replace("*", "")
        );
    ...
    getIndexes()
      .then((indexes) => {
        ...
        if (localStorage.getItem("hasFutures") === "true") {
          const fLastOrderIndexes = getFuturesLastOrderIndexes(
            automation.symbol
          );
          filteredIndexes.push(...fLastOrderIndexes);

          const liquidationIndexes = getFuturesLiquidationIndexes(
            automation.symbol
          );
